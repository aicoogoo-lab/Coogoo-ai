...
║      النسخة النهائية – متوافقة مع واجهة غرفة العرش                       ║
║                                                                      ║
║  ╔══════════════════════════════════════════════════════════════════╗ ║
...
if not SOVEREIGN_KEY:
    logger.warning("⚠️ لم يتم ضبط SOVEREIGN_KEY — المصادقة بالمفتاح لن تعمل. تأكد من متغيرات Railway.")
    SOVEREIGN_KEY = "MASTER_SOVEREIGN_KEY_ULTIMATE"  # Fallback للتوافق
...
app.config.update(
    ...
    SESSION_COOKIE_SECURE=False,  # Railway يتعامل مع HTTPS لكن داخليًا HTTP
...
def require_master(f):
    ...
    - أو Master Header للطلبات البرمجية (X-Master-Key)
    - إذا فشل كلاهما: JSON 401 لطلبات API، أو redirect لـ /login لطلبات الصفحات
    """
    @wraps(f)
    def decorated(*args, **kwargs):
        ...
        if _has_valid_master_header():
            return f(*args, **kwargs)
        # طلبات API ترجع JSON خطأ
        if request.path.startswith("/api/") or request.path in ["/command", "/reason", "/simulate", "/optimize", "/preserve", "/awaken", "/shutdown", "/restart"]:
            return jsonify({"success": False, "error": "غير مصرح. مفتاح السيد مطلوب في X-Master-Key."}), 401
        # طلبات الصفحات ترجع redirect لصفحة الدخول
...
@app.route("/")
def index():
    """الصفحة الرئيسية – غرفة العرش (بدون تسجيل دخول إجباري)."""
    return render_template("index.html")


@app.route("/login", methods=["GET", "POST"])
def login_page():
    """بوابة الدخول (اختيارية – تستخدم للمصادقة عبر المتصفح)."""
    if request.method == "POST":
        password = (request.form.get("password") or "").strip()
        key = SOVEREIGN_KEY.strip()

        if not key:
            return render_template("login.html", error="SOVEREIGN_KEY غير مضبوط في الخادم."), 500
        ...
        if locked_until:
            try:
                ...
                if datetime.now() < until:
                    return render_template("login.html", error="البوابة مغلقة مؤقتًا. افتح /logout لتصفير الجلسة."), 429
            except Exception:
        ...
        if attempts >= 7:
            ...
            return render_template("login.html", error="تم قفل البوابة مؤقتًا (10 دقائق). افتح /logout لتصفير الجلسة."), 429
...
# ═══════════════════════════════════════════════════════════════════════
# المسارات المحمية (تتطلب X-Master-Key أو جلسة is_master)
# ═══════════════════════════════════════════════════════════════════════
